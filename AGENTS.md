...
- **Git & Workflow:** Ogni volta che l'utente invia un task con ID (es. `SHBC-11`), creare immediatamente il branch partendo da `develop`: `feature/<TASK_ID>-<slug>`. Commit message rigorosamente nel formato: `<TASK_ID> <descrizione>` (es. `SHBC-29 setup local tesseract ocr and presidio sdk in celery worker dockerfile with spacy model`).
